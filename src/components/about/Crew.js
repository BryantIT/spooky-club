...
const Crew = () => {
  const { cards, cardsItem, card, cardImage, cardContent, cardTitleWrapper,cardTitle, cardText } = styles
  const crewMembers = [
    {
      image: bryantImage,
...
<ul className={cards}>
  {
    crewMembers ? crewMembers.map(member => (
      <li className={cardsItem}>
        <div className={card}>
          <div className={cardImage} style={{ backgroundImage: `url(${member.image})` }}></div>
